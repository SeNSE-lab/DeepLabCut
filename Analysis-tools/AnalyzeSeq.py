...
Modified by NEB 20181042 to run on Seq files
"""
...
for video in videos:
    ...
    try:
        ...
        print("Loading ", video)
        clip = pims.open(video)
        clip.size = clip.frame_shape
        ny, nx = clip.size
        fps = clip.frame_rate
        #nframes = np.sum(1 for j in clip.iter_frames()) #this is slow (but accurate)
        nframes_approx = clip.header_dict['allocated_frames']
        clip.duration = nframes_approx/fps
        ...
        if cropping:
            raise Exception('Seq processing does not allow for cropping as of
                            20181024')
            clip = clip.crop(
        ...
        for index in tqdm(range(nframes_approx)):
            image = img_as_ubyte(clip.get_frame(index))
